...
2. **État d'implémentation** — Les trois features de ce fichier sont développées en TDD et vérifiées de bout en bout contre la base réelle (25 assertions couvrant chaque policy RLS, y compris les cas limites d'accès croisé entre utilisateurs non liés) :
   ...
   - "Notification de co-ownership" (US8.2) : notification in-app (section "Invitations reçues" + badge numérique) **et** push, via un trigger DB sur `dog_owners` (`status = 'pending' AND role = 'co-owner'`) qui appelle l'API Expo Push directement — voir `modele-de-donnees.md` §Notifications push. Texte non spécifié dans le brief initial, choisi lors de l'implémentation : "🐾 {Inviteur} t'invite à co-gérer {Chien}".
3. **Sexe du chien** — champ ajouté après coup (pas dans le brief initial). Optionnel, deux valeurs (`male`/`female`), même traitement que race/date de naissance : modifiable par n'importe quel owner/co-owner, donnée partagée. Choix binaire délibéré (cohérent avec l'usage vétérinaire/pedigree) — pas d'option "autre" au MVP. Affiché sur la carte de "Mes chiens" (`DogCard`) à côté de la race et de l'âge quand renseigné ("Labrador · 3 ans · Mâle") — capturé dès le départ dans le formulaire mais resté sans affichage nulle part jusqu'à ce que ça soit repéré après coup ; pas montré dans les vues plus compactes (photo confirmée sur une balade, sélecteur de chien) où race/âge n'apparaissent pas non plus.
...
7. **Rafraîchissement au changement d'onglet** — pattern générique (`useRefetchOnFocus`), pas spécifique au domaine dog — voir `walk.docs.md` point 13 pour le détail (bug découvert et corrigé là, React Navigation ne démonte pas les écrans d'onglets donc le refetch-on-mount de TanStack Query ne se déclenchait jamais en y revenant).
