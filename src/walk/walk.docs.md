...
Feature: Liste et détail des balades
  ...
  Scenario: Consulter mes balades à venir
    ...
    And elles sont regroupées sous des en-têtes "Aujourd'hui" / "Demain" / "Cette semaine" / "La semaine prochaine" / "Plus tard", chaque groupe n'apparaissant que s'il contient au moins une balade
...
2. **État d'implémentation** — TDD sur le domaine/l'application, vérifié de bout en bout contre la base réelle :
   ...
   - "Liste et détail des balades" (US5.1) : liste des balades à venir (organisées ou invité), détail avec participants/chiens confirmés. Le détail est maintenant live (Supabase Realtime, voir "Mise à jour en direct des réponses" ci-dessus et point 12 plus bas) ; la liste elle-même reste sur pull-to-refresh/refocus (`useRefetchOnFocus`, voir point 13) plutôt que du Realtime — un changement d'onglet suffit, pas de tirer-pour-rafraîchir manuel. La liste des balades est l'écran par défaut de l'app (premier onglet de la barre, accessible directement à "/") — il n'y a pas de page d'accueil séparée.
...
12. **Mise à jour en direct du détail de balade (Realtime)** — `useWalkRealtime` (`walk/presentation/hooks/`) souscrit aux changements Postgres sur `walk_participants` et `walk_dogs`, filtrés sur le `walk_id` courant, uniquement pendant que `WalkDetailScreen` est monté. Plutôt que de reconstruire l'état à partir du payload brut de l'événement, la souscription se contente d'invalider le cache TanStack Query du `useWalk` correspondant — le merge participants × chiens × mon statut (`findById`) reste la seule source de vérité pour cette forme de données, pas de logique dupliquée côté Realtime. Nécessite que les deux tables soient ajoutées à la publication `supabase_realtime` (migration `20260922213000_walk_detail_realtime.sql`) ; leur clé primaire composite inclut déjà `walk_id`, donc la `REPLICA IDENTITY` par défaut suffit à filtrer les événements sans configuration supplémentaire. RLS s'applique normalement à Realtime : un client ne reçoit que les événements des balades qu'il peut déjà consulter. Scope volontairement limité au détail de balade — ni la liste des balades, ni une reprogrammation (`walks.location_text`/`start_time`/`duration_minutes`) en direct, qui n'étaient pas dans le scénario Gherkin d'origine.
13. **Rafraîchissement au changement d'onglet** — bug découvert en testant le point précédent avec deux comptes : la liste des balades ne se mettait à jour ni au changement d'onglet, ni en revenant sur l'app, seul un pull-to-refresh manuel le faisait. Cause : React Navigation garde les écrans d'onglets montés en permanence (pas de démontage/remontage au switch), donc le refetch-on-mount par défaut de TanStack Query ne se déclenche jamais en revenant sur un onglet déjà visité. Fixé par un hook partagé, `useRefetchOnFocus` (`shared/hooks/`), branché sur `useFocusEffect` (réexporté par `expo-router`) — silencieux, sans le spinner de `usePullToRefresh`, et sautant le tout premier focus (déjà couvert par le fetch initial du montage). Appliqué aux quatre écrans de liste concernés : `WalksListScreen`, `PastWalksListScreen`, `MyDogsScreen`, `FriendsScreen`.
14. **Sexe du chien affiché sur "Chiens confirmés"** — le sexe (voir `dog.docs.md` point 3) était capturé et affiché sur la fiche du chien, mais jamais dans le contexte d'une balade. `WalkDog` (entité domain) et la requête `walk_dogs` (repository) portent maintenant `sex`, jusqu'à l'anneau de la photo elle-même (`DogPhoto`'s nouveau prop `sex`) : plutôt qu'un badge séparé ou un emoji à côté du nom, l'anneau est redessiné en `Svg` (`react-native-svg`, même technique que l'anneau pointillé du foyer partagé) pour former directement le glyphe Mars (♂, flèche haut-droite) ou Vénus (♀, croix en bas) — le cercle et le symbole ne font qu'un, dans la couleur du ring par défaut (`$warning`/amber, identique dans les deux thèmes). Vérifié en rendant le tracé SVG isolément en PNG (`@resvg/resvg-js`, déjà présent pour la génération des icônes) avant intégration, pour confirmer la géométrie sans dépendre du simulateur. Un `accessibilityLabel` explicite ("Rex, mâle") sur l'élément englobant porte l'info pour les lecteurs d'écran plutôt que de compter sur la forme seule. Scope limité au détail de balade — pas ajouté à `WalkCard` (liste des balades), qui ne montre aucun chien individuellement aujourd'hui, ni aux autres usages de `DogPhoto` (sélecteur de chien, pastilles du RSVP).
15. **Regroupement de la liste des balades par date** — `groupWalksByDate` (`walk/presentation/`, testé en TDD) partitionne les balades déjà triées par date en 5 groupes simples : "Aujourd'hui", "Demain", "Cette semaine", "La semaine prochaine", "Plus tard" (fourre-tout au-delà). "Cette semaine"/"La semaine prochaine" suivent la semaine calendaire française (lundi → dimanche), pas une fenêtre glissante de 7 jours — mais "Aujourd'hui"/"Demain" sont vérifiés en premier, donc "demain" reste "demain" même quand il tombe sur le lundi d'une semaine ISO suivante (dimanche → lundi). Groupes vides omis plutôt qu'affichés avec un état vide (ex: pas de section "Demain" s'il n'y a rien demain). `WalksListScreen` passe de `FlatList` à `SectionList` pour ça — `PastWalksListScreen` (l'historique) n'est pas concerné, hors scope de la demande initiale.
16. **Bug de navigation corrigé — pile dupliquée après une reprogrammation** — `WalkEditScreen.handleSubmit` faisait `router.replace(/walks/${walkId})` après un enregistrement réussi. Cet écran n'est atteint que par un `push` depuis `WalkDetailScreen` (menu "•••" → "Modifier") : la pile est donc `[..., Détail, Edit]`, et `replace()` échange `Edit` contre une **nouvelle** entrée `Détail` plutôt que de dépiler vers celle déjà en-dessous — la pile devient `[..., Détail, Détail]`. Un swipe-back depuis l'écran obtenu retombait donc sur cette deuxième instance de la fiche balade au lieu de revenir à la liste. Remplacé par `router.back()`, qui dépile simplement vers l'instance déjà existante ; `useUpdateWalk`'s `onSuccess` avait déjà injecté la balade mise à jour dans le cache de cette instance, donc l'affichage reste à jour de toute façon. `WalkFormScreen` (création) a un `router.replace("/")` du même genre mais vers la racine d'onglet, pas un écran de détail poussé — pas le même mécanisme, pas reproduit le bug signalé, laissé tel quel.
